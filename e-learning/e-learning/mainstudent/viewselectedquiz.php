<?php
  ...
  if(!issset($_POST['file'])){
    header('location:../');
  }
  else{
    $file = $_GET['file'];
    $res = $con->query
    ("SELECT * FROM tbl_quiz INNER JOIN tbl_subject ON tbl_quiz.scode = tbl_subject.scode WHERE tbl_quiz.file = '$file'");
    $row = mysqli_fetch_assoc($res);
  }
?>
<!DOCTYPE html>
<html>
  ...
  <body>
    ...
    <div id="main">
      <a class="btn btn-info btn-md" href="viewquizpage.php">
      <i class="fas fa-arrow-left"></i> Back</a><br><br>
      <div class="card" style="height:500px;">
        <div class="card-header" align="center">
          <b><?php echo $row['sname'].' '. $row['quiz'].': ', $row['chapter']; ?></b>
        </div>
        <div class="card-body">
          <embed type="application/pdf" src="../uploads/quizzes/<?php echo $file; ?>" width="100%" height="100%">
        </div>
      </div>
    </div>
