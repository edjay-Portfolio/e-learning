        ...
        <div class="row">
          ...
          <div class="col-sm">
            <div class="card">
              ...
              <div class="card-body">
                <form action="../script/updateannouncement.php" method="POST" autocomplete="off">
                  <?php
                    $qry = $con->query("SELECT * FROM `tbl_announcement` WHERE `ID`= '$ID'");
                    $data = $qry->fetch_assoc();
                  ?>
                  <div class="form-group">
                    <label for="chapterId">Subject</label>
                    <input id="chapterId" class="form-control" type="text" name="subject" required value="<?php echo $data ['subject']; ?>">
                  </div>
                  <div class="form-group">
                    <label for="chapterId">Message</label>
                    <textarea class="form-control" type="text" name="info" placeholder="<?php echo $data ['info']; ?>" rows="5" required></textarea>
                  </div>
                   <div class="form-group">
                    <label for="chapterId">Posted by</label>
                    <input id="chapterId" class="form-control" type="text" name="editedby" readonly value="<?php echo $row['fname'].' '.$row['sname']; ?>">
                  </div>
                  <div id="msg"></div>
                  <div class="form-group">
                    <input class="btn btn-success btn-block" type="submit" name="submit" value="Edit">
                  </div>
                </form>
